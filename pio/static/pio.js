var Paul_Pio = function (prop) {
    ...
    const tools = {
        ...
        isMobile: () => {
            ...
            const mobileUa = ua.includes("mobile") || ua.includes("android") || ua.includes("ios");

            return window.innerWidth < 500 || mobileUa;
        }
    }
    ...
    this.init = (noModel) => {
        ...
        if (!tools.isMobile()) {
            ...
            if (!current.prevWidth) {
                current.prevWidth = canvas.width;
                current.prevHeight = canvas.height;
            }

            canvas.width = current.prevWidth * ratio;
            canvas.height = current.prevHeight * ratio;

            canvas.style.width = current.prevWidth + "px";
            canvas.style.height = current.prevHeight + "px";
        }
